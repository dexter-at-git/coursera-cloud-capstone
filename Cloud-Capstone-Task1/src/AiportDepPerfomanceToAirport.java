...
public class AiportDepPerfomanceToAirport extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new AiportDepPerfomanceToAirport(), args);
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job jobA = Job.getInstance(conf, "Airport destination");
		...
		jobA.setJarByClass(AiportDepPerfomanceToAirport.class);
		...
		jobB.setMapperClass(DestinationAirportDelayMap.class);
		jobB.setReducerClass(DestinationAirportDelayReduce.class);
	...
	public static class OriginDestPerfomanceMap extends
		...
		public void map(Object key, Text value, Context context)
			...
			TextPair originDestinationPair = new TextPair(new Text(
					originAirport), new Text(destinationAirport));
	...
	public static class DestinationAirportDelayMap extends
		...
		public void map(Text key, Text value, Context context)
			...
			TreeSet<Pair<Float, String>> delayToDestinationMap = new TreeSet<Pair<Float, String>>();
			delayToDestinationMap.add(new Pair<Float, String>(departureDelay,
					destination));
			if (delayToDestinationMap.size() > TopNumber) {
				delayToDestinationMap.remove(delayToDestinationMap.first());
			}

			for (Pair<Float, String> item : delayToDestinationMap) {
	...
	public static class DestinationAirportDelayReduce extends
		...
		public void reduce(Text key, Iterable<TextArrayWritable> values,
			...
			for (TextArrayWritable val : values) {
				...
				String destination = destinationDelayPair[0].toString();
				Float delay = Float.parseFloat(destinationDelayPair[1]
						.toString());
				delayToDestinationMap.add(new Pair<Float, String>(delay,
						destination));
				if (delayToDestinationMap.size() > TopNumber) {
					delayToDestinationMap.remove(delayToDestinationMap.first());
				}
